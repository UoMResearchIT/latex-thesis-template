...
% It's best to use biber, not bibtex, but you'll need to modify the LateX to PDF profile as follows:
% Change the "Path to BibTeX" executable, to the biber executable, located in the same directory, then remove the "backend=bibtex8" phrase
\usepackage{silence}																																% This package suppresses an error about footnotes
\WarningFilter{biblatex}{Patching footnotes failed.}																% It is generated in error and can be ignored
\usepackage[citestyle=authoryear-icomp,sorting=nyt,bibstyle=authoryear]{biblatex}		% This defines how references are handled
% \usepackage[citestyle=authoryear-icomp,sorting=nyt,bibstyle=authoryear]{biblatex}	% See above for instructions
